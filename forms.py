...
from assessment.models import Answer, Question, Response
from assessment.signals import template_completed
...
class ResponseForm(models.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.steps_count = len(self.template.questions.all())
        # add a field for each survey question, corresponding to the question
        # type as appropriate.
        data = kwargs.get("data")
        for i, question in enumerate(self.template.questions.all()):
    ...
    def _get_preexisting_response(self):
        ...
        if not self.user.is_authenticated:
            return None
        try:
            return Response.objects.get(user=self.user, template=self.template)
        except Response.DoesNotExist:
            LOGGER.debug(
                "No saved response for '%s' for user %s", self.template, self.user
            )
            return None
    ...
    def next_step_url(self):
        if self.has_next_step():
            context = {"id": self.template.id, "step": self.step + 1}
            return reverse("template-detail-step", kwargs=context)
        else:
            return None

    def current_step_url(self):
        return reverse(
            "template-detail-step", kwargs={"id": self.template.id, "step": self.step}
        )
    ...
    def save(self, commit=True):
        ...
        for field_name, field_value in list(self.cleaned_data.items()):
            if field_name.startswith("question_"):
                ...
                answer.save()
        template_completed.send(sender=Response, instance=response, data=data)
        return response
